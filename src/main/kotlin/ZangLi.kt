 ...
 */
const val specialDaysString = "[[[16,-21]],//铁虎年满意月\n" +
...
val specialDays = parse3DArray(cleanseText())
var startDate = parse("1951-01-08")//藏历铁虎年十二月(满意月)初一  | 公历1951年2月7日，藏历铁兔年正月(神变月)初一
var endDate= parse("2051-02-11")   //藏历铁马年十二月(满意月)三十
...
data class ZLResult(
    ...
    var tMonth: String = "",
    var day: String = "",
    var dayLeap: Boolean= false,
    var monthLeap: Boolean= false,
    var dayMiss: Boolean= false,
    var value: String = "",
...
/*方法说明
 *@method getZangli
 *@param{LocalDate} dateToLookup 可以转换成标准日期的入参
 *@return {
 * @value 藏历日期
 * @extraInfo 附加信息
 * @month 藏历月份信息
 ...
 * }
*/
fun getZangli(dateToLookup: LocalDate): ZLResult? {
    val dayCountSinceFirstCalendarDay = startDate.until(dateToLookup, DAYS).toInt()
    var countingDays=0
    var countingMonth=0
    specialDays.forEachIndexed { tYear, monthList ->
        var leapMonths = 0//这一年前面闰了几个月
        var cMonth = 0
        while( cMonth < monthList.size) {
            var tDays = 30
            monthList[cMonth].forEach {
                when {
                    it < 0 -> tDays--
                    it > 0 -> tDays++
                    else -> leapMonths++
            ...
            if ((countingDays + tDays) <= dayCountSinceFirstCalendarDay) { //还没到当前月，直接累加日子
                countingDays += tDays
            } else {
                var tDayLeap = false
                var tDayMiss = false
                var tMonthLeap = false
                var tDays = dayCountSinceFirstCalendarDay - countingDays
                monthList[cMonth].forEach { mValue ->
                    if (mValue == 0) {//闰月
                        tMonthLeap = true
                    } else when {
                            mValue + 1 == -tDays -> { tDayMiss = true; tDays++ }//当天缺日
                            mValue == tDays -> { tDayLeap = true; tDays-- }//当天闰日
                            mValue in 1 until tDays -> tDays-- //前面出现一个闰日
                            mValue < 0 && -mValue - 1 < tDays -> tDays++ //前面出现一个缺日
                        }
                }
                if (tYear == 0) {  cMonth = 12 - monthList.size } //adjust month for first year
                val idx = floor((tYear / 2).toDouble() % 5).toInt()
                return ZLResult().apply{
                    year = "" + "铁水木火土"[idx] + "虎兔龙蛇马羊猴鸡狗猪鼠牛"[(tYear % 12)]
                    month = (if (tMonthLeap) "闰" else "") + listOf("正", "二", "三", "四", "五", "六", "七", "八", "九", "十", "十一", "十二")[cMonth-leapMonths]
                    tMonth = (if (tMonthLeap) "闰" else "") + listOf("神变", "苦行", "具香", "萨嘎", "作净", "明净", "具醉", "具贤", "天降", "持众", "庄严", "满意")[cMonth-leapMonths]
                    day = (if (tDayLeap) "闰" else "")+listOf("初一", "初二", "初三", "初四", "初五", "初六", "初七", "初八", "初九", "初十", "十一", "十二", "十三", "十四", "十五", "十六", "十七", "十八", "十九", "二十", "廿一", "廿二", "廿三", "廿四", "廿五", "廿六", "廿七", "廿八", "廿九", "三十")[tDays]
                    dayLeap = tDayLeap
                    monthLeap = tMonthLeap
                    dayMiss = tDayMiss
                    value = year + "年" + month + "月(" + tMonth + "月)" + day
                    (if (!tDayLeap) getExtraInfo(tDays,cMonth) else Pair("","")).apply { extraInfo = first ; extraInfo2 = second }
                }
            }
            cMonth++
        }
...
val eclipsDetails = HashMap<String,EclipseDay>()
val justDateFormat = DateTimeFormatter.ofPattern("yyyy-MM-dd")!!

val justTimeFormat = DateTimeFormatter.ofPattern("H点mm分")!!
private fun formatTime(time:Long): String?  = justTimeFormat.format(Instant.ofEpochMilli(time + ms_oneday * 8 / 24).atZone(ZoneId.systemDefault()).toLocalDateTime())

fun addE(eDay: EclipseDay) {
    val day = justDateFormat.format(LocalDate.ofEpochDay(Duration.ofMillis(eDay.date).toDays()))
    val d: LocalDateTime = Instant.ofEpochMilli(eDay.date).atZone(ZoneId.systemDefault()).toLocalDateTime()
    val info = "食甚" + d.hour + "点" + d.minute + "分"
    val duration = if (eDay.startTime != null) {
        eDay.extraInfo2 = info
        val start = formatTime(eDay.startTime)
        val end = formatTime(eDay.endTime!!)
        "初亏:$start-复圆:$end"
    } else info
    duration.also { eDay.extraInfo = it }
    eclipsDetails[day] = eDay
}


fun parseAEclipseDataToList(s :String): List<Long> = s.removeSurrounding("[","]").replace(" ","").split(",").map { it.toLong() }
fun cleanseText() = specialDaysString.replace("//.*".toRegex(),"").replace("\n","").trim()

fun main() {
    val newS = eclipsDaysString.removeSurrounding("[", "]").replace("],[", "];[")
    newS.split(";").map {
        with(parseAEclipseDataToList(it)) {
            addE(EclipseDay(this[0], eclipseType[this[1].toInt()], this.getOrNull(2), this.getOrNull(3), "", ""))
        }
    }
    val d = "2021-11-19"
    val targetDate = parse(d)
    val r = getZangli(targetDate)
    println("$targetDate $r  <===> " + eclipsDetails[d])
}

fun parse3DArray(specialDaysS: String) = ArrayList<List<List<Int>>>().apply {
        val l = specialDaysS.removeSurrounding("[", "]").replace("]],[[", "]];[[")
        l.split(";").map {
            add(parse2DArray(it))
        }
    }

fun parse2DArray(s: String) = ArrayList<List<Int>>().apply {
    val l = s.removeSurrounding("[", "]").replace("],[", "];[")
    l.split(";").map { a ->
        val sl = a.removeSurrounding("[", "]").trim()
        add(
            if (sl.isNotEmpty())
                sl.split(",").map { it.toInt() }
            else
                emptyList()
        )
    }
